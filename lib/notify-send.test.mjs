...
describe("notify-send module", () => {
  ...
  it("defaults soundFile to the shared ogg when sound is true", () => {
    assert(
      source.includes("DEFAULT_NOTIFICATION_SOUND"),
      "module should define a shared default notification sound constant",
    );
    assert(
      source.includes("task-complete-change.ogg"),
      "default sound should point to task-complete-change.ogg",
    );
    const sendBlock = source.match(/export async function sendNotification[\s\S]*?\n\}/)?.[0] ?? "";
    assert(
      sendBlock.includes("options.sound && !options.soundFile"),
      "sendNotification should fill soundFile when sound is true and soundFile is missing",
    );
    assert(
      sendBlock.includes("DEFAULT_NOTIFICATION_SOUND"),
      "sendNotification should use DEFAULT_NOTIFICATION_SOUND as the fallback",
    );
  });

  it("测试模式给标题与正文打标记并静音", () => {
    assert(source.includes("PI_NOTIFY_TEST"), "should read PI_NOTIFY_TEST");
    assert(
      source.includes('TEST_NOTIFY_TITLE_PREFIX = "[测试] "'),
      "test mode should prefix the title with [测试]",
    );
    assert(
      source.includes("TEST_NOTIFY_MESSAGE = "),
      "test mode should define a message marker line",
    );
    const markBlock = source.match(/function withTestMark[\s\S]*?\n\}/)?.[0] ?? "";
    assert(
      markBlock.includes("message: TEST_NOTIFY_MESSAGE"),
      "test mode should replace the body with the marker line",
    );
    assert(markBlock.includes("sound: false"), "test mode must silence the sound");
    assert(
      markBlock.includes("soundFile: undefined"),
      "test mode must drop the sound file, otherwise paplay still fires",
    );
  });
});
