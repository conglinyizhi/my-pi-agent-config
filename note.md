...
# 20260822-LLM 审核链路重构 + GUI 联动

- 因为弹窗实在是太多了，因为一些正则命中的原因，导致对话框实在是太多。所以添加了一个让 LM 自动审计指令的能力，他如果认为是健康的，那就放行。他如果判定有危险，那么我来进行二次审核。预计这样能省下不少的精力，而且就算他没有通过审核，也会提供一个指示。这样的话我的压力会小很多
- 一开始大模型自己写的 prompt 是让审计模型自己输出一个 json 或者是用代码块包裹的 json 这种情况下虽然稳定，但是不太对劲。然后后来我就让他用工具去了。用工具的话，然后他要是想有一些额外输出也可以用来辅助我决策

# 20260822-审核模型池 + 踩坑记录

- 智谱 glm-4.7-flash 免费但限流（错误 1305 访问量过大），单模型顶不住。
- 但是考虑到免费模型其实不好找，所以我也在着急的去找一些免费的模型 API 去用，或者是考虑直接本地部署，但是 CPU 推理又是一个问题。CPU 推理只能去跑一些小模型，但是其智能我真的有点担忧
- 无论如何，为了提升稳定性以及未来方便未来扩展，我设计了一套模型池，如果有需要的话，直接可以从模型池里面去选择对应的模型去用，失败了也可以从里面再挑另外一个模型去分配一下流量

配套指令（不用手抄供应商名/模型名）：

- /provider:fast-put [关键词]：从全局模型列表（modelRegistry.getAll()）筛选一个加进池子，可选测试链路
- /provider:fast-pop [provider/model]：从池子踢一个

- 然后我意识到，如果将这些东西直接塞入提交仓库的话，会让它依赖断裂。因为它有一些依赖的是模型，另外一些依赖的是 API key。嗯，而这些东西没有办法和算库一起走，同同步到其他电脑上也可能会出现一些问题。所以我就考虑把它单独存放，并且另外保存了一个文件，根据 Git 忽略文件，它应该在这里：extensions/sandbox-permissions/review-pool.toml
