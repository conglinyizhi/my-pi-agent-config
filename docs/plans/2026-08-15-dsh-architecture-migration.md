...
| 4 | **jobs 工具组** | `dsh-jobs` + `dsh-jobs-local` + `dsh-tool-jobs`：`job_output/job_list/job_kill` + 完成通知（wakeup/quiet） | **无**（pi 无后台任务原语） | 先建统一后台任务注册表（内存，重启即失——照抄 DSH「进程本地状态不持久化」）；bash 后台/子 agent 后台形态统一接入；完成通知经 `ctx.ui.notify` + `sendUserMessage` 投递 | 中 | ✅ `extensions/dsh-jobs/`（2026-08-15） |
...
3. **第三批**（#4，✅ 已完成 2026-08-15）：`extensions/dsh-jobs/`——内存注册表（registry.ts：start/list/get/read/kill/wait/onJobDone，reported 位 first-wins）+ bash 后台提供方（providers.ts：spawn 子进程，kill→killed，输出上限截断）+ 四工具（tools.ts：bash_background/job_output/job_list/job_kill）+ 完成通知（index.ts：notify 用户 + wakeup 空闲开新轮次，settings dshJobsDelivery）。15 项单测全过。
