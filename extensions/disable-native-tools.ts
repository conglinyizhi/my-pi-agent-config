...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    const hasRequiredBeTools = requiredBeTools.every((toolName) => allTools.some((tool) => tool.name === toolName));
    ...
    if (hasBeTools && hasRequiredBeTools) {
      ...
      // 过滤掉要禁用的原生工具
      const activeTools = allTools.filter((tool) => !nativeToolsToDisable.includes(tool.name)).map((tool) => tool.name);
      ...
      // 通知用户
      if (ctx.hasUI) {
        ctx.ui.notify(`检测到 BetterEditTools 系列工具，已禁用原生工具：${nativeToolsToDisable.join(", ")}（保留 bash）`, "info");
      }
    }
  });
}
