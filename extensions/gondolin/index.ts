 ...
 * Gondolin 工具路由示例
 *
 * 在本地 Gondolin 微型虚拟机中运行 pi 的内置工具。宿主机工作目录会
 * 挂载到 guest 中的 /workspace。/workspace 下的文件变更会回写到宿主机；
 * guest 文件系统中的其他变更则仅隔离在虚拟机内部。
 *
 * 安装：
...
async function executeGondolinGrep(
	...
	if (matchCount === 0) return { content: [{ type: "text", text: "未找到匹配项" }], details: undefined };
	...
	if (matchLimitReached) {
		...
		notices.push(`已达到 ${effectiveLimit} 条匹配上限`);
	}
	if (linesTruncated) {
...
export default function (pi: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		ctx?.ui.setStatus("gondolin", ctx.ui.theme.fg("accent", `Gondolin：正在启动 ${GUEST_WORKSPACE}`));
		...
		ctx?.ui.notify(`Gondolin VM 已就绪。${localCwd} 已挂载到 ${GUEST_WORKSPACE}。`, "info");
		return created;
	}
	...
	pi.on("session_shutdown", async (_event, ctx) => {
		...
		vmStarting = undefined;
		if (!activeVm) return;
		ctx.ui.setStatus("gondolin", ctx.ui.theme.fg("muted", "Gondolin：正在停止"));
		try {
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		const guestLine = `当前工作目录：${GUEST_WORKSPACE}（Gondolin VM；宿主工作区来自 ${localCwd} 的挂载）`;
