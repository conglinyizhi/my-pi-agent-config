...
- 能列成选项的字段就不用敲：思考返回格式是 10 个格式的单选，输入模态是文本 / 图像 / 视频 / 声音四项独立勾选（当前 pi 只把文本和图像交给模型，视频和声音会写进配置留给其它插件），API 格式、开关类字段、保护级别也全是选择
- 数字字段支持单位与千位分隔：`1M` / `512K` / `1.5M` / `100万`、`1_000_000`、`1,000,000` 都行，不带后缀时保留小数（价格用）；输入框下面会实时把当前输入换算出来（敲 `1M` 就看到 `= 1,000,000（1.0M）`），菜单里的大数字也顺带标出 `1.0M` / `384K`
...
供应商 / 模型 / 字段这类可能很长的列表（选项超过 5 项且处于 TUI 模式）走 `lib/vim-select.ts` 的自绘选择器，其余短菜单仍用 pi 内置选择器。字段编辑对话框里候选较多的也一样（如「思考返回格式」的 10 个格式）。
...
| `%` | 跳到当前会话正在用的供应商；已进入该供应商的模型列表时跳到当前模型。当前项行尾会标 `%`。过滤模式下 `%` 当普通字符 |
