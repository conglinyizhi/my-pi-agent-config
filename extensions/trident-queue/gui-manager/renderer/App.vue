...
const selected = computed(() => tasks.find((t) => t.id === selectedId.value) || null);
...
function toggleCheck(id: string) { checked.has(id) ? checked.delete(id) : checked.add(id); }

function statusIcon(s: string) {
  const m: Record<string, string> = { pending: "○", executing: "▶", done: "✓", blocked: "⏸" };
  return m[s] || "○";
}
function statusColor(s: string) {
  const m: Record<string, string> = { pending: "#7aa2f7", executing: "#9ece6a", done: "#565f89", blocked: "#e0af68" };
  return m[s] || "#565f89";
}
function statusLabel(s: string) {
  const m: Record<string, string> = { pending: "待执行", executing: "执行中", done: "已完成", blocked: "已阻塞" };
  return m[s] || s;
}

function batch(op: "start" | "kill") {
  const ops = [...checked].map((taskId) => ({ taskId, op }));
  try { fs.writeFileSync(rsp, JSON.stringify({ action: "batch", ops })); } catch {}
  window.close();
}
function killOne() {
  if (!selected.value) return;
  try { fs.writeFileSync(rsp, JSON.stringify({ action: "batch", ops: [{ taskId: selected.value.id, op: "kill" }] })); } catch {}
  window.close();
}
function startOne() {
  if (!selected.value) return;
  try { fs.writeFileSync(rsp, JSON.stringify({ action: "batch", ops: [{ taskId: selected.value.id, op: "start" }] })); } catch {}
  window.close();
}
</script>
