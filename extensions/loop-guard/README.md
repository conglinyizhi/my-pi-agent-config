...
- **warn**：状态栏 + 一条提示，不动输出
- **abort**：中止本次生成（已生成的部分保留在会话里，只截断不回滚），等 agent 停歇后
  注入一条纠正消息，带 `triggerTurn` 直接起新回合，让模型带着指令继续干活

注入消息长这样（末尾那层垃圾见下）：

```
<loop_guard>
检测到重复输出，已中止本次生成：重复内容共 9078 字符 / 4539 行，集中在 1 种短句上
（"好。" / "（输出）"），占窗口内 98% 的行。这是在重复占位句，没有产出新信息。
...
<random-trash-word>
轴瓦 ferry 卡箍 colophon 溶洞 indigo gantry …（4000 字符）
</random-trash-word>
</loop_guard>
```
...
模型此刻正卡在重复里，注意力已经塌到一个短循环上。实测高熵随机中英混合串是把它
拽出来的有效手段（原本用在 for-grok-4-5 的续跑消息上，现抽成 `lib/random-filler.ts`）。

摆位契约：**垃圾在最后、与指令之间恰好一个空行**。前半是可执行指令，必须保持完整可读；
空行加元素边界是让模型自己分辨「前半有用、后半是噪声」。填充每次重新生成（固定串会被
模型记住并当成已知噪声忽略）。

这段垃圾只喂给模型；人看到的是渲染器折叠后的两行卡片（`registerMessageRenderer`），
展开才显示全文。
