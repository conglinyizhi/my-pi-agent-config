<template>
  <div v-if="ready" class="app">
    ...
    <footer class="actions">
      ...
      <button data-name="todo-clear" class="btn btn-clear" @click="selected = new Set(); note=''">清空</button>
...
const todos = computed(() => raw.value.filter(item => !item.done));
const done = computed(() => raw.value.filter(item => item.done));
