...
func TestDecideFirstWins(t *testing.T) {
	...
	env, err := h.Decide("req-1", byGUI, nil, "allow", "ok", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = h.Decide("req-1", byAdapter, &Principal{Channel: "x", UserID: "1"}, "deny", "", nil)
...
func TestAdapterNeedsAllowlist(t *testing.T) {
	...
	_, err := h.Decide("req-2", byAdapter, &Principal{Channel: "im", UserID: "u1"}, "allow", "", nil)
