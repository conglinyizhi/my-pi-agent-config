...
**custom-providers** — `/provider fast-add` 快速加模型供应商，`/provider reload` 重载配置；`/provider fast-edit` 交互改配置，`/provider fast-edit-with-copy` 复刻已有模型到别的供应商再微调。长列表用 `lib/vim-select.ts`：`8j` / `3k` 计数跳转、`/` 模糊过滤（行尾标英文别名）；数字输入用 `lib/prompt-with-preview.ts` 实时换算 `1M` → `1,000,000`。
